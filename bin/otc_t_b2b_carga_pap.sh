...
VAL_ESQUEMA=$(mysql -N <<<"select valor from params where entidad = '"$ENTIDAD"' and ambiente='"$AMBIENTE"' AND parametro = 'VAL_ESQUEMA';")
VAL_QUEUE=$(mysql -N <<<"select valor from params where entidad = '"$ENTIDAD"' and ambiente='"$AMBIENTE"' AND parametro = 'VAL_QUEUE';")
